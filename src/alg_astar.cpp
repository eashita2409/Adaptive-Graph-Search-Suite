...
SearchResult AStar::run(const Graph& graph, int sourceId, int targetId) {
    SearchResult res;
    res.algorithmName = "A* Search";
    ...
    gScore[sourceId] = 0.0;
    fScore[sourceId] = heuristic(nodes.at(sourceId), targetNode);
    ...
    pq.push({fScore[sourceId], sourceId});
        ...
        for (const auto& edge : graph.getEdges(u)) {
            ...
            if (visited.find(v) == visited.end()) {
                ...
                if (tentative_gScore < gScore[v]) {
                    ...
                    fScore[v] = gScore[v] + heuristic(nodes.at(v), targetNode);
                    pq.push({fScore[v], v});
